...
const FrederickPage = ({ data }) => (

  <Layout data={data}>
    <SEO title="Frederick" keywords={[`gatsby`, `application`, `react`]} />
    <div className="main-container">
      <div className="left-container">
        <div className="blackbox"><h1 className="headline1">Frederick</h1></div>
        ...
        <div className="blackbox"><h1 className="headline1">Peotry</h1></div>
        <div className="artist-blog-post">
          <div className="artists-images-container">{data.allWordpressPage.edges.map(post =>
            <div>
              <div>
                {post.node.acf && post.node.acf.youtube_iframe &&
                  <div className="artists-youtube-iframe-poetry" dangerouslySetInnerHTML={{ __html: post.node.acf && post.node.acf.youtube_iframe }} />}
              </div>
            </div>
          )}
          </div>
...
export const query = graphql`
  query {
    allWordpressPage(filter: {template: {eq: "tpl-artists.php"}, title: {eq: "Frederick"}}) {
      edges {
        node {
          featured_media {
            localFile {
