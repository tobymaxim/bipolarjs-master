...
const RasvalPage = ({ data }) => (

  <Layout data={data}>
    <SEO title="Rasval" keywords={[`gatsby`, `application`, `react`]} />
    <div className="main-container">
      <div className="left-container">
        <div className="blackbox"><h1 className="headline1">Rasval</h1></div>
...
export default RasvalPage


export const query = graphql`
  query {
    allWordpressPage(filter: {template: {eq: "tpl-artists.php"}, title: {eq: "Rasval"}}) {
      edges {
        node {
          featured_media {
            localFile {
    ...
    allTribeEvents(sort: {order: ASC, fields: start_date}, filter: {categories: {elemMatch: {name: {eq: "Rasval"}}}}) {
        edges {
          node {
            title
            description
